              ...
              <AnimatePresence>
                {isServicesOpen && (
                  <motion.div
                    ...
                    className="absolute top-full -left-20 w-[450px] bg-gradient-to-br from-[#0f172a]/95 to-[#020617]/95 backdrop-blur-xl border border-cyan-500/20 p-6 rounded-2xl shadow-[0_0_30px_rgba(6,182,212,0.15)] grid grid-cols-2 gap-4"
                  >
                    {serviceLinks.map((item, idx) => (
                      <Link key={idx} to={item.route} className="group flex items-center gap-3 p-3 rounded-xl bg-white/5 hover:bg-gradient-to-r hover:from-cyan-500/10 hover:to-blue-600/10 border border-white/5 hover:border-cyan-500/30 transition-all duration-300">
                        <span className="text-xl drop-shadow-md">{item.icon}</span>
                        <div>
                          <div className="text-gray-100 text-[10px] font-bold group-hover:text-cyan-400 uppercase tracking-wider transition-colors">
                            {item.title}
                          </div>
                          <div className="text-[9px] text-cyan-100/50 group-hover:text-cyan-100/80 uppercase tracking-tighter transition-colors">View Details</div>
                        </div>
                      </Link>
                    ))}
                  </motion.div>
              ...
              <AnimatePresence>
                {isPortfolioOpen && (
                  <motion.div
                    ...
                    className="absolute top-full left-1/2 -translate-x-1/2 w-[600px] bg-gradient-to-br from-[#0f172a]/95 to-[#020617]/95 backdrop-blur-xl border border-cyan-500/20 rounded-2xl p-6 shadow-[0_0_30px_rgba(6,182,212,0.15)] grid grid-cols-3 gap-4"
                  >
        ...
        {isMobileMenuOpen && (
          ...
          >
            <div className="flex flex-col space-y-6">
              <Link to="/" className="block w-full text-2xl font-bold text-white uppercase tracking-widest border-b border-white/10 py-4">Home</Link>

              {/* Mobile Services Accordion */}
              <div>
                <button
                  onClick={() => setMobileDropdown(mobileDropdown === 'services' ? null : 'services')}
                  className="w-full flex justify-between items-center text-2xl font-bold text-white uppercase tracking-widest border-b border-white/10 py-4"
                >
                  Services
                ...
                <AnimatePresence>
                  {mobileDropdown === 'services' && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden flex flex-col space-y-4 pt-4 pl-4">
                      {serviceLinks.map((item, idx) => (
                        <Link key={idx} to={item.route} className="flex items-center gap-3 text-gray-300 hover:text-cyan-400 py-2">
              ...
              <Link to="/technologies" onClick={(e) => handleScrollToSection(e, '/technologies', 'technologies')} className="block w-full text-2xl font-bold text-white uppercase tracking-widest border-b border-white/10 py-4">Technologies</Link>

              <Link to="/blogs" onClick={(e) => handleScrollToSection(e, '/blogs', 'blogs')} className="block w-full text-2xl font-bold text-white uppercase tracking-widest border-b border-white/10 py-4">Blogs</Link>
              <Link to="/feedback" className="block w-full text-2xl font-bold text-white uppercase tracking-widest border-b border-white/10 py-4">Feedback</Link>
              <Link to="/about" className="block w-full text-2xl font-bold text-white uppercase tracking-widest border-b border-white/10 py-4">About</Link>
